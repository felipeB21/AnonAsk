...
const avatarVariants = cva(
  "relative box-content flex items-center justify-center overflow-hidden",
  {
    variants: {
      shape: {
        circle: "rounded-full",
...
const avatarTextVariants = cva(
  "flex max-w-full items-center justify-start whitespace-nowrap font-medium text-white",
  {
    variants: {
      size: {
        medium: "text-sm",
...
const Root = ({
  ...
  >
    ...
    {icon && !text && (
      <div className={avatarIconVariants({ size })}>{icon}</div>
    )}
    {text && !icon && (
      <span className={avatarTextVariants({ size })}>{text}</span>
    )}
  </div>
);
